#!/usr/bin/env python
"""Generate per-fold macro-averaged OvR ROC curves for BRCA models."""
...
for fold_idx, fold in enumerate(folds):
    ...
    for model_key, model_name, color in zip(model_keys, models, colors):
        pred_file = os.path.join(
            metrics_dir, f"{model_key}_brca_fold{fold}_predictions.npz"
        )
...
plt.suptitle(
    "Per-Fold Macro-Averaged One-vs-Rest ROC Curves — GS-BRCA",
    fontsize=14,
    fontweight="bold",
    y=0.995,
)
...
# Save
output_path = os.path.join(plots_dir, "roc_brca_models_per_fold.png")
...
# Print summary
print("\n=== Mean AUC Summary ===")
for model_key, model_name in zip(model_keys, models):
    if model_key in auc_data:
        mean_auc, std_auc = auc_data[model_key]
        print(f"{model_name:20s}: {mean_auc:.4f} ± {std_auc:.4f}")
