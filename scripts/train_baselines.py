"""
MLOmics XGBoost Baseline Training Script
==========================================
Trains XGBoost early-fusion baselines with 5-fold CV for all cancer types.

Usage:
    python scripts/train_baselines.py           # Full data
    python scripts/train_baselines.py --toy     # Toy data (BRCA only)

Artifacts produced:
    models/baseline_xgb/xgb_{cancer}_fold{i}.pkl
    results/metrics/xgb_{cancer}_metrics.csv
    results/plots/confusion_matrix_xgb_{cancer}.png
...
from src.evaluation import compute_metrics, print_metrics_table, save_metrics
...
def compute_class_weights(y_train: np.ndarray) -> np.ndarray:
    ...
    Uses sklearn's 'balanced' strategy: weight = n_samples / (n_classes * class_count).
    """
...
def plot_best_fold_confusion_matrix(
    ...
    """Generate and save a confusion matrix heatmap for the best fold.
    ...
    ax.set_title(f"Confusion Matrix: {model_name} ({cancer_type}) — Best Fold", fontsize=13)

    png_path = os.path.join(save_dir, f"confusion_matrix_{model_name.lower()}_{cancer_short}.png")
...
def train_xgboost_cv(
    ...
    Follows the pipeline: load folds -> prepare data -> concatenate ->
    train with sample weights -> evaluate -> save model -> log experiment.
    """
    ...
    for fold_info in folds:
        ...
        # Prepare data with leakage prevention (impute+scale on train only)
        result = prepare_fold_data(cancer_type, fold_info, config, use_toy=use_toy)

        # Concatenate modalities for early fusion
    ...
    # Leakage check
    from src.evaluation import compute_fold_summary
    ...
    if summary["f1_mean"] > 0.95:
        ...
        print("\n!!! WARNING: F1 < 0.40 — Check data alignment, class weights, preprocessing !!!")
...
def main():
    ...
    if args.toy:
        # Toy data only has BRCA
        print("Running on TOY data (BRCA only)")
        train_xgboost_cv("GS-BRCA", config, use_toy=True)
    else:
        for cancer_type in config["project"]["cancer_types"]:
            train_xgboost_cv(cancer_type, config, use_toy=False)
